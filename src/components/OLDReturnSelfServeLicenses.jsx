...
export default function ReturnSelfServeLicenses(props) {
  ...
  function releaseLicense(event, useremail, license) {
    ...
    const url = `https://laxcoresrv.buck.local:8000/release_self_service_license?product=${license.toLowerCase()}&email=${useremail}`;
    ...
    xhr.onload = function() {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        console.error("License return error:", xhr.statusText);
        alert(`Error returning license: ${xhr.statusText}`);
      }
    };

    xhr.onerror = function() {
      setPrevisible(false);
      console.error("License return failed");
      alert("Error returning license. Network error or CORS issue.");
    };
        ...
        queries: [
          {
            ...
            queryFn: async () => {
              return new Promise((resolve, reject) => {
                try {
                  const xhr = new XMLHttpRequest();
                  xhr.open('GET', 'https://laxcoresrv.buck.local:8000/buckokta/category/att/comparison/match?_category=users', true);
                  // xhr.setRequestHeader('x-token', 'a4taego8aerg;oeu;ghak1934570283465g23745693^$&%^$#$#^$#^#$nrghaoiughnoaergfo');
                  xhr.setRequestHeader('Content-Type', 'application/json');
                  xhr.setRequestHeader('Accept', 'application/json');
                  xhr.withCredentials = false; // Changed to false to avoid CORS credentials issue
